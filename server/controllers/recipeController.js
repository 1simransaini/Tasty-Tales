...
exports.submitRecipeOnPost = async(req, res) => {
    try {
        ...
        let uploadPath = require('path').join(__dirname, '/public/uploads/', newImageName);

        imageUploadFile.mv(uploadPath, function (err) {
            if (err) {
                console.error('Error uploading file:', err);
                req.flash('infoErrors', err);
                return res.redirect('/submit-recipe'); // Redirect on error
            }
        });
        ...
        res.redirect('/submit-recipe'); // Redirect on error
    }
  }
